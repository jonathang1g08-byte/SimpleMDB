...
(async function initActorMovieIndex() {
	...
	try {
		...
		if (items.length === 0) {
			renderStatus(statusEl, 'warn', 'No actor-movie credits found for this page.');
		} else {
			...
			for (const am of items) {
				...
				root.querySelector('.info').textContent = `Actor ID: ${am.actorId}, Movie ID: ${am.movieId}`;
				root.querySelector('.btn-view').href = `/ActorMovie/view.html?id=${encodeURIComponent(am.id)}`;
				root.querySelector('.btn-edit').href = `/ActorMovie/edit.html?id=${encodeURIComponent(am.id)}`;
		...
		renderStatus(statusEl, 'err', `Failed to fetch actor-movie credits: ${err.message}`);
	}
})();
